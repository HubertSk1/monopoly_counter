...
for cnt2 in contours2:
    ret = cv2.matchShapes(cnt1,cnt2,1,-1000)
    if  ret<0.5:
...
#green,blue,yellow,red,purple,grey
...
#Dodatkowo Klasyfikacja Neuronowa:
model = keras.models.load_model('cnn2.h5')
img_cnn=cv2.imread(path,cv2.IMREAD_REDUCED_COLOR_2)
img_cnn=cv2.resize(img_cnn,(800,1000))
img_cnn=cv2.cvtColor(img_cnn, cv2.COLOR_BGR2HSV)
img_cnn[drawing6_Centers!=255]=0
...
for point in Mid:
    ...
    Xmax=int(point[1]+radius+2)
    Xmin=int(point[1]-radius-2)
    img_to_pred=img_cnn[Ymin:Ymax,Xmin:Xmax]
    (H,S,V) = cv2.split(img_to_pred)  
    H=cv2.resize(H, (32,32))
    S=cv2.resize(S, (32,32))
    V=cv2.resize(V, (32,32))
    img_to_pred=cv2.merge([H,S,V])
    images.append(img_to_pred)
    
#normalizacja wartosci do przedzialu <0,1>
images=np.array(images)/255

#Wprowadzenie do modelu:
dlugosc=len(images)
pred=model.predict(images[:dlugosc])
etykiety=[]
for predykcja in pred:
    n=0
...
for point in Mid:
    if point[3]==0:
        ...
        Kolor=colors[3]
    img_classified=cv2.circle(img_classified, [int(point[1]),int(point[0])], radius+8,Kolor,4 )

#WYSWIETLENIE WYNIKOW
print(f'Suma pieniedzy to :{Suma}')
